...
- `$sigee-spec-author` is the default first stop for new work: tickets start in `Backlog` with `Next Action` pointing to `$sigee-spec-author`.
- Roles do not coordinate by private messages; they update the ticket and set `Next Action` to the next role.
...
- Implementer: implements only from the spec package/docs; does not read original reference source code.
